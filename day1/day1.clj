...
(defn solve-p2 [path]
  (let [[left right :as input] ;; bind left, right, input using destructuring see: https://clojuredocs.org/clojure.core/let#example-542692c7c026201cdc3269c4
       (->> (get-input path)
       (apply map list))]
    (let [counts (seq-count right)] ;; bind map of counts from right list to counts using seq-count 
